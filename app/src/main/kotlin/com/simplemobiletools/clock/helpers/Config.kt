...
class Config(context: Context) : BaseConfig(context) {
    ...
    /*var stopwatchMilliseconds: Long
        get() = prefs.getLong(STOPWATCH_MILLISECONDS, 0)
        set(lastTimerSeconds) = prefs.edit().putInt(TIMER_SECONDS, lastTimerSeconds).apply()*/
    ...
    var stopwatchLabel: String?
        get() = prefs.getString("stopwatch_label", null)
        set(label) = prefs.edit().putString("stopwatch_label", label).apply()


    var stopwatchChannelId: String?
        get() = prefs.getString(STOPWATCH_CHANNEL_ID, null)
        set(id) = prefs.edit().putString(STOPWATCH_CHANNEL_ID, id).apply()
}
